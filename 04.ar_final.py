"""Two windows from a single capture loop:
    "ar_axes"    = camera view with a 3D cuboid drawn on the chessboard
    "ar_topdown" = rectified top-down view with robot (x, y) labels at every
                   grid intersection

One capture / one chessboard detection / one solvePnP per frame, shared
...
def main():
    ...
    print(f"[ar_final] camera RMS={float(cam_data['rms']):.3f}px  "
          f"theta={np.degrees(theta):+.2f}deg")

    # Inner-corner objp for solvePnP + cuboid (in mm, origin at first inner corner)
    objp = build_inner_object_points((cols, rows), sq_mm)
    box_3d = build_box_points((cols, rows), sq_mm, box_height_mm)

    # Outer grid (15x11 for 13x9 inner)
    n_outer_i = cols + 2
    n_outer_j = rows + 2
    ...
    print(f"[ar_final] measuring first {LOCK_FRAMES} frames then locking pose. "
    ...
    while True:
        ...
        # Detect + solvePnP only during the measurement phase.
            ...
            if found:
                ...
                if ok_pnp:
                    ...
                    if len(rvec_samples) >= LOCK_FRAMES:
                        ...
                        print(f"[ar_final] pose locked after {LOCK_FRAMES} samples.")
        else:
            found = True   # purely for HUD text
        ...
        # RIGHT first: warp the CLEAN frame so the cuboid never appears here.
        if locked_Hmat is not None:
            right = cv2.warpPerspective(frame, locked_Hmat, (Wt, Ht))
            draw_labels(right)
            hud_r = (f"unit={unit}  z={z_board:.4f}m  "
                     f"theta={np.degrees(theta):+.1f}deg  [LOCKED]")
        else:
            right = np.zeros((Ht, Wt, 3), np.uint8)
            hud_r = f"measuring {len(rvec_samples)}/{LOCK_FRAMES}..."
        cv2.putText(right, hud_r, (10, 22), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(right, hud_r, (10, 22), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (0, 255, 255), 1, cv2.LINE_AA)

        # LEFT: live camera + locked cuboid (drawn AFTER the warp above).
        left = frame
        ...
        cv2.putText(left, f"{fps:5.1f} FPS  {status}",
