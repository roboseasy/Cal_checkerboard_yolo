...
### 2b) `02b.board_index.py` — 어느 코너가 `(0,0)` 인지 확인
```bash
python 02b.board_index.py
```
다음 단계에서 EE로 찍을 점은 외곽 격자 인덱스 `(i, j)` 로 적는데, **어느 물리적
코너가 `(0,0)` 이고 +X가 어느 방향인지는 미리 알 수 없습니다.** 그 순서는
`findChessboardCorners` 가 정하고, 보드가 화면에 놓인 방향에 따라 달라집니다.
엉뚱한 코너를 찍으면 04의 RMS는 멀쩡한데 좌표가 통째로 뒤집히거나 90° 돌아갑니다.

- 외곽 격자점을 카메라 화면에 투영해 그리고, **원점을 큰 원으로, +X/+Y를 화살표로** 표시
- 3단계에서 찍을 5개 점은 빨간 원 + `(i,j)` 라벨로 강조
- 키: `a` = 모든 격자점에 라벨, `s` = 스냅샷 저장, `q` 종료

여기서 본 코너 위치를 그대로 3단계에서 찍으면 됩니다.

### 3) `03a.touch_points.py` — EE로 보드 코너 측정 → `config.yaml` 기록
...
```
- 찍을 점을 `(i, j)` 와 설명으로 하나씩 안내하고, EE 좌표 `x y z` (m) 를 받습니다
- `s` = 이 점 건너뛰기, `q` = 저장 없이 종료
- **mm 로 잘못 넣으면 잡아냅니다** (`0.2483` 대신 `248.3` 같은 입력)
- 다 모으면 **분포를 검사**합니다 — 한 줄에 몰려 있으면 회전이 결정되지 않으므로 경고
- `config.yaml` 의 `touch_points` 블록만 갈아끼웁니다 (주석은 그대로 보존, 이전 내용은 `config.yaml.bak`)
- 측정한 z 평균으로 `z_board_m` 갱신 여부를 물어봅니다
- 마지막에 `03.robot_calib.py show` 를 돌려 잔차를 바로 보여줍니다
...
> **EE 좌표(x, y, z)를 읽는 것 자체는 이 저장소가 하지 않습니다.**
> 관측 스트림에는 관절 각도만 오고 EE 좌표가 없어서, 순기구학(FK)이 따로 필요합니다.
> Physical Labs 앱의 EEF 패널처럼 팔을 freedrive 하고 EE 위치를 m 단위로 보여주는
> 도구로 값을 읽어 `03a.touch_points.py` 에 입력하세요.
>
> **freedrive 를 하려면 `lekiwi_host` 를 내려야 합니다.** 토크를 끄려면 모터 버스
> (`/dev/ttyACM0`) 를 직접 잡아야 하는데 host 가 점유하고 있고, ZMQ 프로토콜에는
> 토크 끄기 명령이 없습니다. 3단계는 카메라를 안 쓰니 문제 없습니다 —
> 측정이 끝나면 host 를 다시 띄우고 04/05 로 넘어가세요.
...
├── config_util.py             # config 로드 + 저장소 기준 경로 해석
├── 00.check_link.py           # 연결 점검 + 카메라 고르기
├── 01.calibrate.py
├── 02.ar_axes.py
├── 02b.board_index.py         # 어느 코너가 (0,0) 인지 확인
├── 03a.touch_points.py        # EE 측정값을 config.yaml 에 기록
├── 03.robot_calib.py
├── 04.ar_final.py
